...
class IndicatorSet:
    ...
    def feed(self, all_candles):
        ...
        self.__feed_SMA_arrays_and_EMA_arrays(all_candles, len(all_candles), last_14_candles[-1].close)
        ...
        if len(self.standard_deviation) >= 1 and len(self.SMA.SMA_20) >= 1:
            self.BB.feed(self.SMA.SMA_20[-1], self.standard_deviation[-1], last_14_candles[-1].close)
    ...
    def __feed_SMA_arrays_and_EMA_arrays(self, all_candles, number_of_candles, last_closing_price):
        try:
            if number_of_candles <= 12:
    ...
    def __define_trend(self, last_close_price):
        if last_close_price < self.EMA.EMA_100[-1]:
